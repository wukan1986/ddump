...
各数据商提供的API只是一个下载数据的通道，但没有解决以下问题：

1. API不是100%稳定，可能因各种原因导致下载失败。需要能重试
2. API有流量限额、时间有限，需要能增量下载
3. 数据可能下载一段时间后会更新，需要能再次下载

## 使用逻辑

1. 将API查询参数分成两部分：时间参数、查询参数
2. 时间参数用于构造本地文件名。`Dump__start__end(api, path, start_name, end_name)`，没有时间参数就用`Dump()`
3. `set_parameters(func_name)`用于设置查询的函数和参数，这决定了保存文件的路径
4. `exists(file_timeout, data_timeout)`判断文件是否存在,只下载`[file_timeout, data_timeout]`之间的数据，
   即才下没多久的不下（防止短时间内重复下载），很久的以前的不下（数据基本不再更新）
5. `download(kw)`执行下载动作，`kw`指定`set_parameters`时，哪些参数需要传给`api`，这里遇到各种问题会重试
6. `save`保存，如果下载的数据非`DataFrame`，可以在这提前处理成`DataFrame`
7. 如果要调试，可以先注释`download(kw)`前的`@retry`

## 大数据量的分批下载

1. `set_parameters+exists`判文件是否要下载更新
2. `more_itertools.batched`将任务分批
3. `set_parameters+download`进行数据下载，这里会将下载的数据进行文件缓存
4. `save`将这多次下载的数据合并，然后保存到指定位置
5. 请参考`examples\jqresearch\get_price_stock_minute.py`示例
